    ...
    class ProjectsController < ActionController::API
      ...
      def provision
        ...
        if platform_project_id.present?
          ...
          project.platform_project_id ||= "rfx_#{SecureRandom.hex(8)}"
      ...
      def ensure_project_keys(project)
        return unless project.settings.is_a?(Hash)

        changed = false
        project.settings["api_key"] ||= begin
          changed = true
          "rfx_api_#{SecureRandom.hex(24)}"
        end
        project.settings["ingest_key"] ||= begin
          changed = true
          "rfx_ingest_#{SecureRandom.hex(24)}"
        end
